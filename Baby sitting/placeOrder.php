...
$worker_id = intval($_POST['worker_id'] ?? 0); // From form POST data
$worker_name = $_POST['worker_name'] ?? '';
$pet_type = $_POST['pet_type'] ?? null;
$place_price = floatval($_POST['place_price'] ?? 0);
$worker_price = floatval($_POST['worker_price'] ?? 0);
$days = intval($_POST['days'] ?? 0); // Number of days from form
$total_cost = floatval($_POST['total_cost'] ?? 0); // Total cost from form
...
// Validate input
if (!$user_id || !$worker_id || !$start_date || !$end_date || !$pickup_location || !$pet_type || $days <= 0 || $total_cost <= 0) {
...
if ($conn->query($order_sql) === TRUE) {
    ...
    $pet_caring_sql = "
        INSERT INTO baby_sitting (age_group, place_price, start_date, end_date, pickup_location, total_cost, order_id)
    ...
    if ($conn->query($pet_caring_sql) === TRUE) {
        ...
        if ($result && $result->num_rows > 0) {
            ...
            if ($conn->query($insert_availability_sql) === TRUE) {
                ...
                header("Location: ../home_page.php"); // Uncomment to redirect
                exit();
            } else {
        ...
        echo "Error in baby sitting: " . $conn->error;
    }
} else {
