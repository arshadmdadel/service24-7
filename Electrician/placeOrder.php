...
$date = $_SESSION['start_date'] ?? null; // Single date to be used for both start and end dates
...
if (!$user_id || !$worker_id || !$date || !$pickup_location || !$pet_type) {
    die("Missing required session data.");
}

// Calculate the total cost (date is not included in cost calculation)
$total_cost = $worker_price + $place_price;
...
if ($conn->query($order_sql) === TRUE) {
    ...
    $pet_caring_sql = "
        ...
        VALUES ('$pet_type', '$place_price', '$date', '$date', '$pickup_location', '$total_cost', '$order_id')
    ";
